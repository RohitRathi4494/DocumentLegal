...
import { FileText, Plus, Trash2, Eye, Download, PlayCircle, Users } from 'lucide-react'
...
export default function CreateDocumentPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setSchema(tplRes.data.form_schema || { sections: [] })
      setClients(clientsRes.data)
      
      // Initialize form data
      const initialData: any = {}
      ;(tplRes.data.form_schema?.sections || []).forEach((section: any) => {
        section.fields.forEach((field: any) => {
          if (field.type === 'repeatable') {
            initialData[field.name] = [{}] // start with 1 empty row
  ...
  const submitDocument = async (action: 'preview' | 'generate') => {
    setProcessing(true)
    try {
      const payload = {
